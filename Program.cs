using Azsign.Modelos.FirmaAcuerdo.Request;
using Azsign.Modelos.FirmaAcuerdo.Response;
using Azsign.Modelos.SolicitudAcuerdoInfo.Request;
using Azsign.Modelos.SolicitudAcuerdoInfo.Response;
using Azsign.Modelos.WebHook;
...
namespace Azsign
{
    class Program
    {
        ...
        static void SolicitarFirmaAcuerdo()
        {
            ...
            XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(acuerdo.GetType());
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var env = EnvelopeSerialize<FirmaAcuerdoResponse>(response);
        ...
        static void SolicitarInfoAcuerdo()
        {
            var acuerdoInfo = new SolicitarAcuerdoInfo()
            {
                ...
                //AcuerdoId = "20241028-103536-df4660-03207321",//Errado
                Aplicativo = "20241022-124027-4fafe1-45992970",
                Cuenta = "20230317-094007-4c9c60-89584564",
                IncluirDocs = true
            };
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        var envelope = EnvelopeSerialize<SolicitudAcuerdoInfoEnvelopeResponse>(response);

                        //Deserealizar la respuesta al tipo Envelope<T>
                        var envelope2 = EnvelopeSerialize<SolicitudAcuerdoInfoEnvelopeResponse>(responseContent);

                        //Deserealizar la respuesta al tipo T
                        var acuerdo = ResponseSerialize<Modelos.SolicitudAcuerdoInfo.Response.AcuerdoInfo>(responseContent);
                    ...
                    else
                    {
                        ...
                        var envelope = EnvelopeSerialize<ErrorEnvelopeResponse>(response);


                        //Deserealizar la respuesta al tipo Envelope<T>
                        var envelope2 = EnvelopeSerialize<ErrorEnvelopeResponse>(responseContent);

                        //Deserealizar la respuesta al tipo T
                        var acuerdo = ResponseSerialize<Errores>(responseContent);
                    }
